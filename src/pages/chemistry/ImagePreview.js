...
const ImagePreview = props => {
  const { imageId, imageUrl, setFile, imagePreviewUrl, setImagePreviewUrl, maxWidth, maxHeight } = props
  ...
  const handleImageChange = e => {
    ...
    if (newFile?.size > maxFileSize) {
      notification.error({ 
        message: "File can't be large than 4MB"
      })
      return
    }
    ...
    <div style={style.mb2}>
      <div style={style.mb2}>
        ...
        <label
          ...
          {imagePreviewUrl || imageUrl ? "Change image" : "Choose file"}
        </label>
      </div>
