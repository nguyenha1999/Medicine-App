...
import { Modal, Form, DatePicker, Row, Col, notification, Input } from "antd"
...
const BillDetail = ({ item, onOk, onCancel }) => {
  ...
  const onChangeSelector = values => {
    ...
    setData({
      ...data,
      products: [...data.products.filter(product => values.includes(product._id)), ...newProducts]
    })
  }
  ...
  >
    <Form form={form} name="control-hooks" onFinish={onFinish}>
      ...
      {
        data.products.map(product => <Form.Item key={product._id}>
          <Row gutter={8}>
            <Col span={12}>
            ...
            <Col span={12}>
              <Input
                ...
                placeholder="Count"
                value={product.count}
                onChange={e => onUpdateCountProduct(product._id, Number(e.target.value))}
              />
            </Col>
          </Row>
        </Form.Item>)
