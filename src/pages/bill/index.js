...
import { get, create, update, remove } from "../../api/bill"
...
const Bill = () => {
  ...
  const updateData = useCallback(async (values) => {
  ...
  const print = record => {
    ...
    doc.autoTable({
      ...
      body: products.map(product => [product._id, product.name, product.count]),
      theme: "grid",
      rowPageBreak: 'avoid'
    })
    doc.save(`${isExport ? "ExportBill" : "ImportBill"}-${moment(createdAt).format(DATE_FORMAT)}.pdf`)
  }
  ...
  return <Layout>
    ...
    <Row style={style.mb2}>
      <Col span={4}>
        <Button color="success" onClick={() => setEditingItem({
      ...
      <Col span={12}>
        <Search placeholder="Search" onSearch={value => setSearch(value)} enterButton />
      </Col>
    </Row>
    <Row>
      <Col span={24}>
          ...
          expandable={{
            expandedRowRender: record => <div>
              ...
              {!!record.products?.length
                ? record.products.map(product => <div style={style.bold}>{product.name}: {product.count}</div>)
